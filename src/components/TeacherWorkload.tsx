...
import { 
  Users, BookOpen, Clock, Filter, Search, Download, BarChart2,
  GraduationCap, Layers, Building2, Calendar, UserCheck, 
  CheckCircle, AlertCircle, Info, LayoutDashboard, ChevronRight, PieChart
} from 'lucide-react';
...
export function TeacherWorkload() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'majors' | 'classes' | 'teachers'>('overview');
  ...
  const majorStats = useMemo(() => {
    ...
    }).sort((a, b) => b['学生人数'] - a['学生人数']).slice(0, 8); // Top 8 majors
  ...
  const classStatusBreakdown = useMemo(() => {
    const total = classMetrics.active || 1;
    return [
      { name: '正常在校', count: classMetrics.inSchool, color: 'bg-emerald-500', textColor: 'text-emerald-700', bgColor: 'bg-emerald-50', percentage: Math.round((classMetrics.inSchool / total) * 100) },
      { name: '外出实习', count: classMetrics.internship, color: 'bg-orange-500', textColor: 'text-orange-700', bgColor: 'bg-orange-50', percentage: Math.round((classMetrics.internship / total) * 100) },
      { name: '实习返校', count: classMetrics.returned, color: 'bg-indigo-500', textColor: 'text-indigo-700', bgColor: 'bg-indigo-50', percentage: Math.round((classMetrics.returned / total) * 100) },
    ];
  }, [classMetrics]);
  ...
  const deptSummaryList = useMemo(() => {
    return state.departments.map(dept => {
      ...
      const teachersInDept = state.teachers.filter(t => t.department === dept.name).length;

      // Calculate subjects directly assigned to this department or its majors
      const deptSubjects = state.subjects.filter(s => 
        s.departmentId === dept.id || (s.majorId && deptMajorIds.has(s.majorId))
      ).length;
  ...
  const filteredClassesList = useMemo(() => {
    ...
    if (classStageFilter !== 'all') {
      result = result.filter(c => (c.stage || '高一') === classStageFilter);
    }
    if (classStatusFilter !== 'all') {
      result = result.filter(c => (c.status || '正常在校') === classStatusFilter);
    }
    ...
    return result.map(c => {
      ...
      const dept = major ? state.departments.find(d => d.id === major.departmentId) : null;
      const headteacher = state.teachers.find(t => t.id === c.headTeacherId);
      ...
      return {
        ...
        deptName: dept?.name || '未知专业部',
        gradeName: grade?.name || (c.stage || '未指定'),
        headTeacherName: headteacher?.name || '未分配',
        totalHours,
        subjectCount
      };
    });
  }, [state.classes, state.majors, state.departments, state.grades, state.teachers, state.schedules, classMajorFilter, classStageFilter, classStatusFilter, classTypeFilter, classSearchQuery]);
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-slate-500 uppercase">在校班级数</span>
              ...
              <div>
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-extrabold text-slate-800">{classMetrics.active}</span>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-slate-500 uppercase">开设专业数</span>
              ...
              <div>
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-extrabold text-slate-800">{state.majors.length}</span>
                  <span className="text-sm text-slate-500">个</span>
                </div>
                <p className="text-[11px] text-slate-400 mt-1">
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-slate-500 uppercase">在校生规模</span>
              ...
              <div>
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-extrabold text-slate-800">{studentMetrics.grandTotal}</span>
                  <span className="text-sm text-slate-500">人</span>
                </div>
                <p className="text-[11px] text-slate-400 mt-1">
                  均班: {classMetrics.active > 0 ? Math.round(studentMetrics.grandTotal / classMetrics.active) : 0} 人/班
                </p>
              </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-slate-500 uppercase">教师总数</span>
              ...
              <div>
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-extrabold text-slate-800">{state.teachers.length}</span>
                  <span className="text-sm text-slate-500">人</span>
                </div>
                <p className="text-[11px] text-slate-400 mt-1">
                  师生比 1 : {state.teachers.length > 0 ? (studentMetrics.grandTotal / state.teachers.length).toFixed(1) : '0'}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-slate-500 uppercase">全校周总课时</span>
                <span className="p-1.5 bg-indigo-50 text-indigo-600 rounded-lg">
              ...
              <div>
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-extrabold text-slate-800">{totalSchoolHours}</span>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm lg:col-span-2">
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={gradeStats} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                    <XAxis dataKey="name" tick={{fontSize: 12}} />
                    <YAxis yAxisId="left" stroke="#6366f1" tick={{fontSize: 11}} label={{ value: '学生人数 (人)', angle: -90, position: 'insideLeft', style: {textAnchor: 'middle', fontSize: 11, fill: '#6366f1'} }} />
                    <YAxis yAxisId="right" orientation="right" stroke="#10b981" tick={{fontSize: 11}} label={{ value: '班级数 (个)', angle: 90, position: 'insideRight', style: {textAnchor: 'middle', fontSize: 11, fill: '#10b981'} }} />
                    <Tooltip contentStyle={{borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'}} />
                    <Legend wrapperStyle={{fontSize: 12}} />
                    <Bar yAxisId="left" dataKey="学生人数" fill="#6366f1" radius={[4, 4, 0, 0]} />
                    <Bar yAxisId="right" dataKey="班级数" fill="#10b981" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div className="border-t border-slate-100 pt-5">
                ...
                <div className="space-y-3">
                  {classTypeBreakdown.length === 0 ? (
                    <p className="text-xs text-slate-400 italic">暂无类型分布</p>
                  ) : (
                    classTypeBreakdown.map((item, idx) => {
                      const colors = ['bg-indigo-500', 'bg-sky-500', 'bg-violet-500', 'bg-pink-500'];
                      ...
                      return (
                        <div key={idx} className="space-y-1">
                          ...
                          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div className={`h-full ${activeColor}`} style={{ width: `${item.percentage}%` }} />
                          </div>
                        </div>
                      );
                    })
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm lg:col-span-2">
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={majorStats} layout="vertical" margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#e2e8f0" />
                    <XAxis type="number" tick={{fontSize: 11}} />
                    <YAxis dataKey="name" type="category" width={110} tick={{fontSize: 11, fill: '#475569'}} />
                    <Tooltip contentStyle={{borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'}} />
                    <Legend wrapperStyle={{fontSize: 12}} />
                    <Bar dataKey="学生人数" name="在校生数" fill="#4f46e5" radius={[0, 4, 4, 0]} barSize={12} />
                    <Bar dataKey="班级数" name="班级个数" fill="#10b981" radius={[0, 4, 4, 0]} barSize={8} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="divide-y divide-slate-100 overflow-y-auto max-h-[260px] pr-1">
                  {deptSummaryList.map((dept, idx) => (
                    <div key={idx} className="py-3 flex items-center justify-between hover:bg-slate-50 transition-colors px-1 rounded-lg">
                      ...
                      <div className="text-right">
                        <span className="font-bold text-sm text-indigo-600 block">{dept.studentCount}人</span>
                        <span className="text-[10px] text-slate-400 block">周课时 {dept.totalHours} 节</span>
                      </div>
                    </div>
                  ))}
                  {deptSummaryList.length === 0 && (
                    <p className="text-xs text-slate-400 italic py-4 text-center">暂无专业部数据</p>
                  )}
                </div>
              </div>
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              <span className="text-xs font-semibold text-slate-500 uppercase block">专业部数量</span>
              <span className="text-2xl font-extrabold text-slate-800 mt-1 block">{state.departments.length} 个</span>
            </div>
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              <span className="text-xs font-semibold text-slate-500 uppercase block">开设专业总数</span>
              <span className="text-2xl font-extrabold text-slate-800 mt-1 block">{state.majors.length} 个</span>
            </div>
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              <span className="text-xs font-semibold text-slate-500 uppercase block">规划总招生指标</span>
              <span className="text-2xl font-extrabold text-slate-800 mt-1 block">
                {state.majors.reduce((sum, m) => sum + (m.enrollmentTarget || 0), 0)} 人
              </span>
            </div>
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              <span className="text-xs font-semibold text-slate-500 uppercase block">实际在校生人数</span>
              <span className="text-2xl font-extrabold text-indigo-700 mt-1 block">{studentMetrics.grandTotal} 人</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 bg-slate-50 border-b border-slate-200">
              <h3 className="font-bold text-slate-800">全校专业分布与教学指标对照表</h3>
              <p className="text-xs text-slate-500 mt-1">展示专业对应的招生指标、班级数、在校生人数及平均班额指标</p>
            </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {majorDetailedList.map((major) => {
                    const achievementRate = major.enrollmentTarget > 0 
                      ? Math.round((major.studentCount / major.enrollmentTarget) * 100)
                      : null;
                      ...
                      <tr key={major.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-center">
                          <div className="flex items-center justify-center gap-1.5 text-xs text-slate-500">
                            {major.inSchoolCount > 0 && <span className="bg-emerald-50 text-emerald-700 px-1 rounded">在校:{major.inSchoolCount}</span>}
                            {major.internshipCount > 0 && <span className="bg-orange-50 text-orange-700 px-1 rounded">实习:{major.internshipCount}</span>}
                            {major.returnedCount > 0 && <span className="bg-indigo-50 text-indigo-700 px-1 rounded">返校:{major.returnedCount}</span>}
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 text-right text-slate-600">
                          {major.enrollmentTarget > 0 ? (
                            <div className="flex flex-col items-end">
                              <span className="font-semibold">{major.enrollmentTarget}人</span>
                              {achievementRate !== null && (
                                <span className={`text-[10px] font-bold ${achievementRate >= 100 ? 'text-emerald-600' : 'text-amber-600'}`}>
                                  达成率 {achievementRate}%
                                </span>
                              )}
                            </div>
                          ) : (
                            <span className="text-slate-400 italic text-xs">未指定</span>
                          )}
                        </td>
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-500">专业方向</label>
                <select
                ...
                >
                  <option value="all">所有专业方向 ({state.majors.length})</option>
                  {state.majors.map(m => (
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-500">教学阶段</label>
                <select
                  value={classStageFilter}
                  onChange={(e) => setClassStageFilter(e.target.value)}
                  className="w-full px-3 py-1.5 border border-slate-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                >
                  <option value="all">所有教学阶段 (高一/高二/高三/毕业)</option>
                  <option value="高一">高一阶段</option>
                  <option value="高二">高二阶段</option>
                  <option value="高三">高三阶段</option>
                  <option value="已毕业">已毕业阶段</option>
                </select>
              </div>

              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-500">在校状态 (课时管理)</label>
                <select
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-500">班级类型</label>
                <select
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <div>
                <h3 className="font-bold text-slate-800">全校班级状态与课时覆盖明细</h3>
                <p className="text-xs text-slate-500 mt-1">
                  当前已筛选出 {filteredClassesList.length} 个班级，包含 {filteredClassesList.reduce((sum, c) => sum + (c.studentCount || 0), 0)} 名学生
                </p>
              </div>
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-100 text-slate-600 font-medium">
                  <tr>
                    ...
                    <th className="px-6 py-3.5 text-right w-36">已排/覆盖课时</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                      ...
                      <tr key={cls.id} className={`hover:bg-slate-50 transition-colors ${isDisabled ? 'bg-slate-50/50' : ''}`}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            <span className="font-medium text-slate-700 text-xs">{cls.majorName}</span>
                            <span className="text-[11px] text-slate-400 mt-0.5">{cls.deptName} • {cls.gradeName}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4 text-xs font-mono text-slate-600">
                          {cls.classroom || <span className="text-slate-400 italic">未指定</span>}
                        </td>
                        <td className="px-6 py-4 text-center font-bold text-slate-700">
                          {cls.studentCount || 0}
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          {isDisabled ? (
                            <span className="text-[11px] font-bold text-slate-400 italic">不排课 (清零)</span>
                          ) : cls.totalHours > 0 ? (
                            <div className="flex flex-col items-end">
                              <span className="font-extrabold text-indigo-600">{cls.totalHours} 节/周</span>
                              <span className="text-[10px] text-slate-400">覆盖 {cls.subjectCount} 门课程</span>
                            </div>
                          ) : (
      ...
      {activeTab === 'teachers' && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-500">产业部</label>
                <select 
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-500">主教科型</label>
                <select 
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-500">年龄区间</label>
                <select 
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-8 mb-6 animate-fade-in">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                  <p className="text-sm font-medium text-indigo-600">本筛选下排课教师</p>
                  <p className="text-3xl font-bold text-indigo-900 mt-1">{teacherDashboardData.activeTeachersCount} <span className="text-sm font-normal text-indigo-700">人</span></p>
                </div>
                <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-100">
                  <p className="text-sm font-medium text-emerald-600">平均工作负荷</p>
                  <p className="text-3xl font-bold text-emerald-900 mt-1">{teacherDashboardData.avgSchoolWorkload} <span className="text-sm font-normal text-emerald-700">课时/周</span></p>
                </div>
                <div className="bg-amber-50 p-4 rounded-lg border border-amber-100">
                  <p className="text-sm font-medium text-amber-600">任课教学课程总数</p>
                  <p className="text-3xl font-bold text-amber-900 mt-1">
                    {teacherDashboardData.totalCourses} <span className="text-sm font-normal text-amber-700">门</span>
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Dept Chart */}
                <div className="h-80">
                  <h3 className="text-sm font-bold text-slate-700 mb-4 text-center">各产业部教师平均周课时与课程数</h3>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={teacherDashboardData.statsByDept} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                      <XAxis dataKey="name" tick={{fontSize: 12}} />
                ...
                {/* Age Chart */}
                <div className="h-80">
                  <h3 className="text-sm font-bold text-slate-700 mb-4 text-center">各年龄段平均周课时与分布人数</h3>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={teacherDashboardData.statsByAge} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                      <XAxis dataKey="name" tick={{fontSize: 12}} />
                      <YAxis />
                      <Tooltip contentStyle={{borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'}} />
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 border-b border-slate-200 text-slate-600">
                <tr>
                  <th className="px-6 py-4 font-semibold w-52">教师姓名与部门</th>
                  <th className="px-6 py-4 font-semibold">授课班级课程与课时明细</th>
                  <th 
                    ...
                    <tr key={teacher.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-[11px] text-slate-400 font-mono mt-0.5">
                              {calculateAge(teacher.idCard) ? `${calculateAge(teacher.idCard)}岁` : '年龄未知'}
                            </span>
                      ...
                      <td className="px-6 py-4">
                        {teacher.schedules.length === 0 ? (
                          <span className="text-slate-400 italic text-xs">暂无任课与课时分配</span>
                        ) : (
                          <div className="space-y-3">
                            {Object.entries(teacher.deptBreakdown).map(([deptName, items], idx) => (
                              <div key={idx} className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                                <div className="text-[10px] font-bold text-slate-400 mb-2 uppercase tracking-wider">{deptName}</div>
                                <div className="flex flex-wrap gap-2">
                                  {(items as any[]).map((item, i) => (
                                    <span key={i} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-white border border-slate-200 text-slate-700 text-xs shadow-sm font-medium">
                                      {item.className}
                                      <span className="text-slate-400 font-normal">|</span>
                                      <span className="text-slate-500 font-semibold">{item.subjectName}</span>
                                      <span className="text-indigo-600 font-extrabold bg-indigo-50 px-1 rounded ml-0.5">{item.hours}节</span>
                                    </span>
                                  ))}
                                </div>
                              </div>
                      ...
                      <td className="px-6 py-4 text-right align-top pt-6">
                        <span className={`inline-flex items-center justify-center px-3.5 py-1 rounded-full font-extrabold text-sm border shadow-sm ${
                          teacher.totalHours > 20 ? 'bg-rose-50 text-rose-700 border-rose-200' :
                          teacher.totalHours > 12 ? 'bg-amber-50 text-amber-700 border-amber-200' :
                          teacher.totalHours > 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
