...
import { Plus, Building2, UserCircle, BookOpen, Tags, Trash2, Edit2, Save, X, GraduationCap, Users } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-4 border-b border-slate-100 bg-slate-50">
              ...
              <form onSubmit={(e) => { e.preventDefault(); if(newTeacherName.trim()) { addTeacher(newTeacherName.trim()); setNewTeacherName(''); } }} className="mt-3 flex gap-2">
                <input type="text" value={newTeacherName} onChange={(e) => setNewTeacherName(e.target.value)} placeholder="新教师姓名..." className="flex-1 px-3 py-1.5 border border-slate-300 rounded-md text-sm" />
                <button type="submit" disabled={!newTeacherName.trim()} className="bg-emerald-600 text-white px-3 py-1.5 rounded-md hover:bg-emerald-700 disabled:opacity-50 text-sm"><Plus className="w-4 h-4" /></button>
              </form>
            </div>
            <div className="p-4 max-h-[400px] overflow-auto">
              <div className="flex flex-wrap gap-2">
                {state.teachers.map(t => (
                  <div key={t.id} className="flex items-center gap-1 bg-slate-100 px-3 py-1.5 rounded-full text-sm border border-slate-200">
                    {t.name}
                    <button onClick={() => deleteTeacher(t.id)} className="text-slate-400 hover:text-rose-600 ml-1"><X className="w-3 h-3" /></button>
                  </div>
                ))}
              </div>
            </div>
