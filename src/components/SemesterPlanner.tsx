...
function getDefaultWeeklyHours(name: string): number {
  if (['普通班', '3+2', '普通班（团队）', '3+2（团队）', '五年制', '春招班'].includes(name)) {
    return 30;
  }
...
export function SemesterPlanner() {
  ...
  const startAddClass = (defaultDeptId?: string) => {
    const firstMajor = majors.find(m => defaultDeptId ? m.departmentId === defaultDeptId : true) || majors[0];
  ...
  const getClassWeeklyHours = (cls: any) => {
    ...
    const category = state.classCategories.find((cc: any) => cc.name === cls.type);
  ...
  }, [departments, majors, classes, teachers, state.classCategories, internshipHours, teacherStandardHours]);

  // 3. Major Forecast Calculations
  const majorForecasts = useMemo(() => {
    return majors.map(m => {
      const majorClasses = classes.filter(c => c.majorId === m.id && !c.isPreset);
  ...
  }, [majors, departments, classes, teachers, state.classCategories, state.subjects, internshipHours, teacherStandardHours]);
  ...
  const subjectForecasts = useMemo(() => {
    ...
    return subjectNames.map((subName, idx) => {
      const matchingSubjects = state.subjects.filter(s => s.name === subName);
      ...
      const dept = sampleSubject?.departmentId ? departments.find(d => d.id === sampleSubject.departmentId) : null;
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-800">在校班级建制与属性管理</h4>
                  <p className="text-[11px] text-slate-500">直接在此模块添加、编辑、删除或通过 Excel 批量导入全校班级，确保排课与规划一体化</p>
                </div>
              </div>
                                ...
                                <tr key={cls.id} className={`hover:bg-slate-50 transition-colors ${cls.isPreset ? 'bg-amber-50/40' : ''}`}>
                                  ...
                                  <td className="px-6 py-4 text-xs">
                                    {cls.isPreset ? (
                                      ...
                                      <span className="text-emerald-600 font-semibold">标准排课 ({clsHours}课时)</span>
                                    ) : status === '外出实习' ? (
                                      <span className="text-orange-600 font-semibold">指导排课 ({clsHours}课时)</span>
                                    ) : (
                                      <span className="text-slate-400">已停用 (0课时)</span>
                                    )}
                                  </td>
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-2">在校/返校班级 周教学总课时 (全局参考)</label>
                  <div className="flex items-center gap-2">
                    <input
                      type="number"
                      value={onCampusHours}
              ...
              <div className="mt-6 pt-6 border-t border-slate-100">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[11px] text-slate-400">修改后将直接实时联动排课系统的算力内核与师资缺口测算</span>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                  {state.classCategories.map(cat => {
                    const value = cat.weeklyHours !== undefined ? cat.weeklyHours : getDefaultWeeklyHours(cat.name);
                    return (
                      <div key={cat.id} className="bg-slate-50 p-2.5 rounded-lg border border-slate-200 flex items-center justify-between hover:border-slate-300 transition-colors">
                        <span className="text-xs font-bold text-slate-700 truncate max-w-[100px]">{cat.name}</span>
                        <div className="flex items-center gap-1 shrink-0">
                          <input
                            type="number"
                            value={value}
                            onChange={(e) => updateClassCategoryHours(cat.id, parseInt(e.target.value) || 0)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-slate-900 text-white p-5 rounded-xl border border-slate-800 shadow-sm flex flex-col justify-between">
                ...
                <span className="text-3xl font-extrabold mt-2 font-mono">
                  {classes.filter(c => !c.isPreset).reduce((sum, c) => sum + getClassWeeklyHours(c), 0)} <span className="text-xs font-normal text-slate-400">节</span>
                </span>
              </div>
              <div className="bg-indigo-50 p-5 rounded-xl border border-indigo-100 shadow-sm flex flex-col justify-between text-indigo-900">
                ...
                <span className="text-3xl font-extrabold mt-2 font-mono">
                  {parseFloat((classes.filter(c => !c.isPreset).reduce((sum, c) => sum + getClassWeeklyHours(c), 0) / teacherStandardHours).toFixed(1))} <span className="text-xs font-normal text-indigo-500">人</span>
                </span>
              </div>
              ...
              <div className={`p-5 rounded-xl border shadow-sm flex flex-col justify-between ${
                parseFloat((classes.filter(c => !c.isPreset).reduce((sum, c) => sum + getClassWeeklyHours(c), 0) / teacherStandardHours - totals.actualTeachers).toFixed(1)) > 0 ? 'bg-rose-50 border-rose-100 text-rose-950' : 'bg-emerald-50 border-emerald-100 text-emerald-950'
              }`}>
                <span className="text-xs font-bold text-slate-500">理论教师缺口数量</span>
                <span className="text-3xl font-extrabold mt-2 font-mono">
                  {parseFloat((classes.filter(c => !c.isPreset).reduce((sum, c) => sum + getClassWeeklyHours(c), 0) / teacherStandardHours - totals.actualTeachers).toFixed(1)) > 0 
                    ...
                    : parseFloat((classes.filter(c => !c.isPreset).reduce((sum, c) => sum + getClassWeeklyHours(c), 0) / teacherStandardHours - totals.actualTeachers).toFixed(1))} <span className="text-xs font-normal text-slate-500">人</span>
                </span>
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-800">多维度师资缺口精准测算</h4>
                  <p className="text-[11px] text-slate-500">
                    {forecastView === 'dept' && '按科室/专业部汇总全校各大板块的师资和课时需求'}
                    {forecastView === 'major' && '下钻精准到各专业，对比招生目标、在校生及相关专业教师'}
                    {forecastView === 'subject' && '对比当前学期已安排的学科课时总数和教师主授学科，精准计算学科缺口'}
                  </p>
                </div>
              </div>
            ...
            {forecastView === 'dept' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                {deptForecasts.map(df => (
                  <div key={df.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4 hover:border-slate-300 transition-colors">
            ...
            {forecastView === 'major' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                {majorForecasts.map(mf => (
                  <div key={mf.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4 hover:border-slate-300 transition-colors">
                    <div className="flex justify-between items-start border-b border-slate-100 pb-3">
                      ...
                      <span className={`text-xs px-2.5 py-0.5 rounded-full font-bold ${
                        mf.gap > 0 ? 'bg-rose-100 text-rose-800' : 'bg-emerald-100 text-emerald-800'
                      }`}>
                        {mf.gap > 0 ? `缺口 ${mf.gap} 人` : `富余 ${Math.abs(mf.gap)} 人`}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-3 gap-x-6 text-xs text-slate-600">
                      <div>
                        ...
                        <p className="text-base font-bold text-slate-800 mt-0.5">{mf.totalClasses} 个班</p>
                        <p className="text-[10px] text-slate-400 mt-0.5">
                          (在校 {mf.onCampusCount} / 实习 {mf.internshipCount} / 返校 {mf.returnedCount})
                        </p>
                      </div>
                      <div>
                        <span className="text-slate-400">测算周总课时需求:</span>
                        <p className="text-base font-bold text-slate-800 mt-0.5">{mf.totalWeeklyHours} 节课</p>
                      </div>
                      <div>
                        <span className="text-slate-400">测算需专任教师:</span>
                        <p className="text-base font-bold text-slate-800 mt-0.5">{mf.teachersNeeded} 人</p>
                      </div>
                      <div>
                        <span className="text-slate-400">目前在册专任教师:</span>
                        <p className="text-base font-bold text-indigo-700 mt-0.5">{mf.actualTeachersCount} 人</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1 pt-2">
                      <div className="flex justify-between text-[10px] text-slate-400 font-bold">
                        <span>师资配置满足率</span>
                        <span>{mf.teachersNeeded > 0 ? Math.round((mf.actualTeachersCount / mf.teachersNeeded) * 100) : 100}%</span>
                      </div>
                      <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-300 ${
                            mf.gap > 1 ? 'bg-red-500' : mf.gap > 0 ? 'bg-amber-500' : 'bg-emerald-500'
                          }`}
                          style={{ width: `${Math.min(100, mf.teachersNeeded > 0 ? (mf.actualTeachersCount / mf.teachersNeeded) * 100 : 100)}%` }}
                        />
                      </div>
                    </div>
                  </div>
            ...
            {forecastView === 'subject' && (
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden animate-fade-in">
                <div className="px-6 py-4 bg-slate-50 border-b border-slate-200">
                  <h4 className="font-bold text-slate-800 text-sm">全校学科课时需求与教师资源精准测算表</h4>
                  <p className="text-xs text-slate-400 mt-1">
                    系统将提取当前学年教学大纲或排课明细内的所有学科已排总课时，对比登记了该主售学科（Primary Subject）的专任教师，提供精准到学科维度的供需对比。
                  </p>
                </div>
                <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {subjectForecasts.map(sf => (
                        <tr key={sf.id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-6 py-4 font-bold text-slate-800">{sf.name}</td>
                          <td className="px-6 py-4">
                            <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${
                              sf.type === '文化基础课' ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'bg-indigo-50 text-indigo-700 border border-indigo-100'
                            }`}>
                              {sf.type}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-right font-semibold font-mono text-indigo-600">{sf.totalWeeklyHours} 节</td>
                          <td className="px-6 py-4 text-right font-medium font-mono text-slate-700">{sf.teachersNeeded} 人</td>
                          <td className="px-6 py-4 text-right font-medium font-mono text-slate-700">{sf.actualTeachersCount} 人</td>
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-2">
                              <div className="flex-1 bg-slate-100 rounded-full h-2 overflow-hidden">
                                <div 
                                  className={`h-full rounded-full transition-all ${
                                    sf.gap > 0 ? 'bg-rose-500' : 'bg-emerald-500'
                                  }`}
                                  style={{ width: `${Math.min(100, sf.teachersNeeded > 0 ? (sf.actualTeachersCount / sf.teachersNeeded) * 100 : 100)}%` }}
                                />
                              </div>
                              <span className="text-xs font-bold text-slate-600 font-mono w-10 text-right">
                                {sf.teachersNeeded > 0 ? Math.round((sf.actualTeachersCount / sf.teachersNeeded) * 100) : 100}%
                              </span>
                            </div>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <span className={`inline-flex px-2 py-1 rounded text-xs font-bold ${
                              sf.gap > 0 ? 'bg-rose-50 text-rose-700 border border-rose-100' : sf.gap < 0 ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-slate-50 text-slate-500'
                            }`}>
                              {sf.gap > 0 ? `缺 ${sf.gap} 人` : sf.gap < 0 ? `余 ${Math.abs(sf.gap)} 人` : '供需平衡'}
                            </span>
                          </td>
                        </tr>
                      ))}
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-slate-500 mb-1">班级名称 <span className="text-red-500">*</span></label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">所属专业 <span className="text-red-500">*</span></label>
                  <select 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">所属年级 <span className="text-red-500">*</span></label>
                  <select 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">班级类型</label>
                  <select 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">教室地点</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">在校人数</label>
                  <input 
                    type="number" 
                    value={classForm.studentCount} 
                    onChange={e => setClassForm({...classForm, studentCount: parseInt(e.target.value) || 0})}
                    className="w-full px-3 py-1.5 border border-slate-300 rounded-lg text-sm focus:ring-1 focus:ring-indigo-500" 
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">入学阶段 (升级自动推进)</label>
                  <select 
                    value={classForm.stage} 
                    onChange={e => setClassForm({...classForm, stage: e.target.value as any})}
                    className="w-full px-3 py-1.5 border border-slate-300 rounded-lg text-sm focus:ring-1 focus:ring-indigo-500"
                  >
                    <option value="高一">高一</option>
                    <option value="高二">高二</option>
                ...
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-slate-500 mb-1">班主任</label>
                ...
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-slate-500 mb-1">班级状态</label>
                  <select 
